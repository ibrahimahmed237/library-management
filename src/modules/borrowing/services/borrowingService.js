...
class BorrowingService {
  async checkoutBook({ book_id, borrower_id, return_date }) {
    ...
    // Start transaction
    const borrowingTransaction = await BorrowingHistory.sequelize.transaction();
    ...
    try {
      ...
      await Book.update(
        ...
        { where: { id: book_id }, transaction: borrowingTransaction }
      );

      await borrowingTransaction.commit();
      return borrowing;
    } catch (error) {
      await borrowingTransaction.rollback();
      throw error;
    }
  }
  ...
  async returnBook(borrowing_id, borrower_id) {
    ...
    // Start transaction
    const returnTransaction = await BorrowingHistory.sequelize.transaction();
    ...
    try {
      ...
      await returnTransaction.rollback();
      throw error;
    }
  }
